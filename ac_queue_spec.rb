...
describe AcQueue do
  ...
  describe "#enqueue" do
    ...
    it "should not add aircrafts with invalid parameters" do
      output = ac_queue.enqueue(Aircraft.new(type: "commercial", size: "medium"))
      expect(output).to eq("Invalid type commercial, Invalid size medium")
      expect(ac_queue.queues.values.all?(&:empty?)).to be(true) 
    end

    it "should not add other instances that aren't aircrafts" do
